...
class EmployementStatusPersonelSeeder extends Seeder
{
    /**
    ...
    public function run(): void
{
    ...
    $csvFile = base_path('database/seeders/ref/ref_employment_status_personnel.csv');
    ...
    // Open and read the file
        if (($handle = fopen($csvFile, 'r')) !== FALSE) {
            ...
            while (($row = fgetcsv($handle, 0, ',')) !== FALSE) { // 0 means no length limit
                ...
                try {
                    DB::table('employment_status_personnels')->insert($data);
                } catch (\Exception $e) {
